...
import { Loader2, Mail, Lock, Eye, EyeOff, ShieldHalf, Sparkles } from "lucide-react"

interface Props {
  onToggleMode: () => void
}

export function AdminLoginForm({ onToggleMode }: Props) {
    ...
    <Card className="relative w-full max-w-md overflow-hidden border border-rose-100/60 bg-white/80 backdrop-blur-xl shadow-2xl rounded-3xl">
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="relative">
            <div className="p-5 rounded-2xl bg-gradient-to-br from-rose-500/20 to-pink-500/20 border border-rose-200/60 shadow-xl">
              <ShieldHalf className="h-10 w-10 text-rose-600" />
            </div>
            <Sparkles className="h-5 w-5 text-rose-400 absolute -top-1 -right-1 animate-pulse" />
          </div>
          <div className="text-center space-y-2">
            <h2 className="text-3xl font-serif font-light tracking-tight text-slate-800">Admin Portal</h2>
            <p className="text-sm text-slate-500">Secure access to management dashboard</p>
          </div>
        </div>
      </CardHeader>
      <CardContent className="relative p-8 pt-2">
        ...
        <div className="mt-8 text-center text-sm text-slate-600">
          <span className="mr-1">Don't have an admin account?</span>
          <button
